...
pub trait IonCliCommand {
        ...
        if !self.is_stable() {
            ...
            if about.is_some() {
                base_command = base_command
                    .about(format!("(UNSTABLE) {}", about.unwrap()))
            }
            base_command = base_command
    ...
    fn run(&self, command_path: &mut Vec<String>, args: &ArgMatches) -> anyhow::Result<()> {
        ...
        match (subcommand.is_stable(), args.get_flag(USE_UNSTABLE_FLAG)) {
            (true, true) => eprintln!(
...
const USE_UNSTABLE_FLAG: &str = "use-unstable";
        ...
        self.show_unstable_flag()
            .arg(
                Arg::new(ION_VERSION_ARG_ID)
                    ...
                    .default_value("1.0")
                    .requires(USE_UNSTABLE_FLAG),
            )
            .mut_arg(USE_UNSTABLE_FLAG, |a| a.hide(false))
    }

    /// All commands have the "unstable" opt-in flag. This makes it visible.
    fn show_unstable_flag(self) -> Self {
        self.mut_arg(USE_UNSTABLE_FLAG, |arg| arg.hide(false))
    }
