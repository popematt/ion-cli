...
pub trait FileSystemWrapper<'a, T: ReadWrite> {
    fn create(&'a mut self, path: &dyn AsRef<Path>) -> io::Result<T>;
    fn open(&'a mut self, path: &dyn AsRef<Path>) -> io::Result<T>;
    fn tempfile(&'a mut self) -> io::Result<T>;
}
...
mod fake_fs {
    ...
    impl <'a> FileSystemWrapper<'a, FakeFile<'a>> for FakeFileSystem<'a> {
...
mod real_fs {
    ...
    impl FileSystemWrapper<'static, File> for RealFileSystem {
